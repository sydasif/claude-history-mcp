...
7 tools, 1 resource — all defined in `server.py` with try/except wrappers returning `[{"error": str(e)}]` on failure:

- `list_sessions`, `get_session_transcript`, `search_messages`, `search_history`, `get_model_usage(include_totals?, session_id?)`, `memory_retain`, `memory_reflect`
- Resource: `claude://health`
