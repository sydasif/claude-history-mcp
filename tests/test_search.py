...
def test_search_messages_finds_text(tmp_path):
    ...
    engine.cache.insert_messages(
        ...
        "s1",
        "s1.jsonl",
        [{"entry_type": "user", "timestamp": None, "uuid": "u1", "content_text": "litellm proxy setup", "raw_json": "{}"}],
    )
...
def test_get_session_full_conversation(tmp_path):
    ...
    engine.cache.insert_messages(
        pid, "sess1", "s.jsonl", [{"entry_type": "user", "timestamp": None, "uuid": "u1", "content_text": "hi", "raw_json": "{}"}]
    )
...
def test_search_history_finds_commands(tmp_path):
    ...
    engine.cache.insert_history_commands(
        [{"display": "search for litellm", "project": "/a", "sessionId": "s1", "timestamp": 1000}]
    )
...
def test_get_recent_activity_includes_null_timestamps(tmp_path):
    ...
    engine.cache.insert_messages(
        ...
        "s.jsonl",
        [
            {"entry_type": "user", "timestamp": recent, "uuid": "u1", "content_text": "recent one", "raw_json": "{}"},
            {"entry_type": "user", "timestamp": None, "uuid": "u2", "content_text": "no timestamp", "raw_json": "{}"},
        ],
    )
...
def test_get_recent_activity_excludes_old(tmp_path):
    ...
    engine.cache.insert_messages(
        pid, "s1", "s.jsonl", [{"entry_type": "user", "timestamp": old, "uuid": "u1", "content_text": "old one", "raw_json": "{}"}]
    )
    result = engine.get_recent_activity(hours=24)
